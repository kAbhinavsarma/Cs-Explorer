...
// Expanded questions database (sample, 5 questions for brevity)
const questions = [
...
// Auto-assign difficulty for all questions that don't have it
function autoAssignDifficulty(q) {
  const text = q.question_text.toLowerCase();
  // Easy: definitions, basic time/space, FIFO/LIFO, basic SQL, basic protocols
  if (
    /time complexity.*binary search|fifo|lifo|queue|stack|primary key|sql stand|dns|ip address|encryption|hash function|linear search|basic|purpose of a hash|main advantage of a hash|node in a linked list|purpose of a switch|web browsing|file transfer|firewall|vpn|two-factor authentication|vulnerability/.test(text)
  ) return 1;
  // Hard: topological sort, deadlock, virtual memory, paging, advanced algorithms, 3NF, foreign key, starvation, round robin, non-preemptive, shortest path, topological, advanced security
  if (
    /topological|deadlock|virtual memory|paging|shortest path|dijkstra|starvation|round robin|non-preemptive|3nf|foreign key|advanced|vulnerability|encryption code|process manager|i\/o manager|file system|resource manager/.test(text)
  ) return 3;
...
questions.forEach(q => {
  if (typeof q.difficulty === 'undefined') {
    q.difficulty = autoAssignDifficulty(q);
  }
});
...
function seedQuestions() {
  ...
  questions.forEach(question => {
    db.run(
      `INSERT INTO questions (question_text, option_a, option_b, option_c, option_d, correct_option, topic, explanation, difficulty) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
